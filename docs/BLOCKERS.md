...
| 12 | **Full 12-month MJP GR backfill** | The GR portal's department/date search is CAPTCHA-gated and its pagination fails viewstate-MAC (load-balanced, no sticky sessions); neither is auto-solved | Incremental discovery of newly published GRs is automated (captcha-free listing). Deep history is ingested from `mjp_store/seeds.json` (explicit codes) or manual assist. |
